...
/// <summary>
/// Made by Diabolickal at Reddit.com
///
/// Do-what-you-want-with-it License :)
/// I am releasing this a Public Domain CC0 License
/// Anyone can use/improve this as they see fit personal/educational/commercial
...
public class MaskMaker : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        else
        {
            ...
            {
                if(!previewMat)
                    previewMat = new Material(Shader.Find("HDRP/Lit"));//Find("HDRenderPipeline/Lit"));
    ...
    private void PackTextures()
    {
        ...
        string s = AssetDatabase.GetAssetPath(Albedo);
        int lastSlash = s.LastIndexOf("/") + 1;

        //var path = EditorUtility.SaveFilePanelInProject("Save Texture To Diectory", "LitMask", "png", "Saved");
        var path = EditorUtility.SaveFilePanel("Save Texture to Directory", Path.GetDirectoryName(s), Albedo.name + "_LitMask", "png"); //Need to use Path.GetDirectoryName(string path) to set the default save directory
